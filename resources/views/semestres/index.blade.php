    ...
    <slot name="header">
        <div class="space-y-2">
            ...
            <p class="text-sm text-[#64748B]">
                Liste et gestion des semestres académiques
            </p>
        </div>
    </slot>
                            ...
                            <tbody class="bg-white divide-y divide-[#D5DBE8]">
                                @forelse ($semestres as $semestre)
                                    <tr class="hover:bg-[#EFF6FF] transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-[#64748B]">{{ $semestre->nom }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <span class="px-2.5 py-0.5 text-xs font-medium rounded-full
                                                    {{ $semestre->statut === 'actif' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200' }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            ...
                                            <form action="{{ route('semestres.destroy', $semestre) }}" method="POST" class="d-inline">
                                                ...
                                                <button type="submit"
                                                        ...
                                                        onclick="return confirm('�Êtes-vous sûr de vouloir supprimer ce semestre ? Il ne peut être supprimé s'il est lié à des modules ou des notes de semestre.');">
                                                    Supprimer
                                                </button>
                                            </form>
                                        </td>
